...
class CustomerList extends Component {
    ...
    fetchCustomers = () => {
        ...
        .then(response => {
            // Assuming the response data is the array of customers
            ...
            <Container>
                ...
                <ListGroup>
                    {customers.map(customer => (
                        <ListGroup.Item key={customer.id} onClick={()=> this.props.onCustomerSelect(customer)} className='d-flex justify-content-between align-items-center shadow-sm p-3 mb-3 bg-white rounded'>
                            {customer.name}
                            <div>
                                <Link to={`/edit-customer/${customer.id}`} className='text-primary'><Button className='me-2'>Edit</Button></Link>
